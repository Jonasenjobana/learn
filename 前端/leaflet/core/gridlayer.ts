        ...
        _setZoomTransform: function (level: any, _center: { lng: number, lat: number }, zoom: number) {
            ...
            if (center != undefined && this.options) {
                if (this.options.corrdType == 'gcj02') {
                    center = SLUMap.togps84gcj02(_center.lng, _center.lat);
                } else if (this.options.corrdType == 'google') {
                    if (SLUMap.isInChina(_center.lng, _center.lat)) {
                        center = SLUMap.togps84gcj02(_center.lng, _center.lat);
                    }
                } else if (this.options.corrdType == 'bd09') {
                    center = SLUMap.togps84bd09(_center.lng, _center.lat);
                }
            }
...
export enum SLEMap {
    ...
    tianDiTuNormalAnnotionE = 'TianDiTu.Normal.AnnotionE',
    ...
    gaoDeNormalAnnotionE = 'GaoDe.Normal.AnnotionE',
    ...
    googleNormalMapE = 'Google.Normal.MapE',
    googleSatelliteMap = 'Google.Satellite.Map',
    googleSatelliteAnnotion = 'Google.Satellite.Annotion',

    cartoNormalAnnotion = 'Carto.Normal.Annotion',
    cartoNormalAnnotionE = 'Carto.Normal.AnnotionE',
 ...
 * @param options 地图配置
 */
export class SLULeafletNetTileLayer extends L.TileLayer {
    constructor(name: SLEMap, options?: SLPMapLeafletLayer) {
        const { url, opts } = SLULeafletNetTileLayer.parseMapProvider(name, options);
        super(url, opts);
    }
    /**将图层添加到map显示在页面 */
    ...
    /**设置map的地图来源，名称，类型 */
    private static parseMapProvider(name: SLEMap, options?: SLPMapLeafletLayer): { url: string; opts: SLPMapLeafletLayer } {
        options = options || {}
        ...
        options.corrdType = SLULeafletNetTileLayer.getCorrdType(mapSource);
     ...
     * @param name 地图来源名称
     */
    private static getCorrdType(name: string) {
        var zbName = "wgs84"
        switch (name) {
            case "Geoq":
            case "GaoDe":
...
const MAPINFO: any = {
    TianDiTu: {
        Normal: {
            ...
            AnnotionE: "//t{s}.tianditu.com/DataServer?T=eva_w&X={x}&Y={y}&L={z}&tk={key}",
            // AnnotionE: "//t{s}.tianditu.gov.cn/eva_w/wmts?SERVICE=WMTS&REQUEST=GetTile&VERSION=1.0.0&LAYER=eva&STYLE=default&TILEMATRIXSET=w&FORMAT=tiles&TILEMATRIX={z}&TILEROW={y}&TILECOL={x}&tk={key}",
        },
        Satellite: {
    ...
    GaoDe: {
        Normal: {
            ...
            AnnotionE: '//webrd0{s}.is.autonavi.com/appmaptile?lang=en&size=1&scale=1&style=8&x={x}&y={y}&z={z}'
        },
        Satellite: {
    ...
    Google: {
        Normal: {
            ...
            MapE: "//www.google.cn/maps/vt?lyrs=m@189&hl=en&x={x}&y={y}&z={z}",
        },
        Satellite: {
    ...
    Carto: {
        Normal: {
            Annotion: "//{s}.basemaps.cartocdn.com/rastertiles/voyager_only_labels/{z}/{x}/{y}{r}.png",
            AnnotionE: "//{s}.basemaps.cartocdn.com/rastertiles/voyager_only_labels/{z}/{x}/{y}{r}.png",
        },
        Subdomains: ['a', 'b', 'c', 'd']
    },
    Baidu: {
